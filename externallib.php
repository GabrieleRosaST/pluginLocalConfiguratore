...
class local_configuratore_external extends external_api {
    ...
    public static function save_chatbot_config($data, $filedata = []) {
        ...
        // DEBUG: Log del risultato della creazione corso
        file_put_contents(__DIR__ . '/debug.log', "DEBUG - Risultato creaCorsoMoodle: " . print_r($risultato_corso, true) . "\n", FILE_APPEND);

        //aggiorno il courseid della tabella configuratore chatbot
        if (!empty($risultato_corso['course']['id'])) {
            $new_courseid = $risultato_corso['course']['id'];
            file_put_contents(__DIR__ . '/debug.log', "DEBUG - Aggiorno courseid da 1 a: $new_courseid per chatbotid: $chatbotid\n", FILE_APPEND);
            
            $update_result = $DB->set_field('local_configuratore_chatbot', 'courseid', $new_courseid, ['id' => $chatbotid]);
            file_put_contents(__DIR__ . '/debug.log', "DEBUG - Risultato aggiornamento courseid: " . ($update_result ? 'SUCCESS' : 'FAILED') . "\n", FILE_APPEND);
            
            // Verifica che l'aggiornamento sia avvenuto
            $updated_record = $DB->get_record('local_configuratore_chatbot', ['id' => $chatbotid]);
            file_put_contents(__DIR__ . '/debug.log', "DEBUG - Record dopo aggiornamento: courseid=" . $updated_record->courseid . "\n", FILE_APPEND);
        } else {
            file_put_contents(__DIR__ . '/debug.log', "DEBUG - ERRORE: risultato_corso non contiene course.id valido\n", FILE_APPEND);
        }
        ...
        // DEBUG: Log finale prima del return
        $final_courseid = isset($risultato_corso['course']['id']) ? $risultato_corso['course']['id'] : null;
        file_put_contents(__DIR__ . '/debug.log', "DEBUG - Return finale: courseid=$final_courseid, redirecturl=$redirecturl\n", FILE_APPEND);
     ...
     *
     * @return array Un array contenente le informazioni dell'utente
     */
    ...
    /**
     * Aggiorna solo i dati base di una configurazione esistente (senza argomenti)
     */
    public static function update_chatbot_basic($chatbotid, $data, $filedata = []) {
        global $DB, $USER;

        error_log("DEBUG update_chatbot_basic - Raw data received: " . print_r($data, true));
        
        self::validate_parameters(
            self::update_chatbot_basic_parameters(),
            ['chatbotid' => $chatbotid, 'data' => $data, 'filedata' => $filedata]
        );
        ...
        // Decodifica i dati JSON se è una stringa
        if (is_string($data)) {
            $dati = json_decode($data, true);
            error_log("DEBUG update_chatbot_basic - Decoded JSON: " . print_r($dati, true));
        } else {
            $dati = $data;
            error_log("DEBUG update_chatbot_basic - Data is not JSON string: " . print_r($dati, true));
        }

        if (!$dati) {
            error_log("ERROR update_chatbot_basic - Failed to decode data");
            return ['success' => false, 'message' => 'Dati non validi'];
        }

        // Verifica che il record esista e appartenga all'utente
        $existing_record = $DB->get_record('local_configuratore_chatbot', 
            array('id' => $chatbotid, 'userid' => $USER->id)
        );
        ...
        $update_data->id = $chatbotid;
        $update_data->nomechatbot = $dati['nomeChatbot'] ?? $existing_record->nomechatbot;
        $update_data->corsochatbot = $dati['corsoChatbot'] ?? $existing_record->corsochatbot;
        $update_data->descrizionechatbot = $dati['descrizioneChatbot'] ?? $existing_record->descrizionechatbot;
        $update_data->istruzionichatbot = $dati['istruzioniChatbot'] ?? $existing_record->istruzionichatbot;
        
        // Converti le date da formato yyyy-mm-dd a timestamp
        if (!empty($dati['dataInizio'])) {
            $update_data->datainizio = strtotime($dati['dataInizio']);
        } else {
            $update_data->datainizio = $existing_record->datainizio;
        }
        
        if (!empty($dati['dataFine'])) {
            $update_data->datafine = strtotime($dati['dataFine']);
        } else {
            $update_data->datafine = $existing_record->datafine;
        }
        
        $update_data->timemodified = time();

        error_log("DEBUG update_chatbot_basic - Final update data: " . print_r($update_data, true));

        try {
            $result = $DB->update_record('local_configuratore_chatbot', $update_data);
            
            if ($result) {
                error_log("SUCCESS update_chatbot_basic - Record updated successfully");
                return ['success' => true, 'message' => 'Configurazione aggiornata con successo'];
            } else {
                error_log("ERROR update_chatbot_basic - Failed to update record");
                return ['success' => false, 'message' => 'Errore durante l\'aggiornamento'];
            }
        } catch (Exception $e) {
            error_log("EXCEPTION update_chatbot_basic - " . $e->getMessage());
            return ['success' => false, 'message' => 'Errore database: ' . $e->getMessage()];
        }
    }

    public static function update_chatbot_basic_parameters() {
        return new external_function_parameters([
            'chatbotid' => new external_value(PARAM_INT, 'ID configurazione da aggiornare'),
            'data' => new external_value(PARAM_RAW, 'Dati JSON dei campi base'),
            'filedata' => new external_multiple_structure(
                new external_single_structure([
                    'itemid' => new external_value(PARAM_INT, 'Draft item ID'),
                    'filename' => new external_value(PARAM_TEXT, 'Nome file'),
                    'filepath' => new external_value(PARAM_TEXT, 'Percorso file'),
                    'mimetype' => new external_value(PARAM_TEXT, 'Tipo MIME'),
                    'argomento' => new external_value(PARAM_TEXT, 'Argomento associato'),
                ]),
                'File da salvare',
                VALUE_DEFAULT, []
            ),
    ...
    public static function update_chatbot_basic_returns() {
        return new external_single_structure([
            'success' => new external_value(PARAM_BOOL, 'Esito'),
            'message' => new external_value(PARAM_TEXT, 'Messaggio'),
        ]);
    ...
    /**
     * Verifica se esiste già un corso con lo stesso nome per l'utente
     */
    public static function check_course_name_exists($coursename, $excludeid = null) {
        global $DB, $USER;

        self::validate_parameters(
            self::check_course_name_exists_parameters(),
            ['coursename' => $coursename, 'excludeid' => $excludeid]
        );
        ...
        // Query per verificare l'esistenza
        $sql = "SELECT COUNT(*) as count FROM {local_configuratore_chatbot} 
                WHERE userid = ? AND corsochatbot = ?" . $exclude_sql;
        ...
        return [
            'exists' => $count > 0,
            'count' => $count,
            'message' => $count > 0 ? 'Corso già esistente' : 'Nome disponibile'
        ];
    }

    public static function check_course_name_exists_parameters() {
        return new external_function_parameters([
            'coursename' => new external_value(PARAM_TEXT, 'Nome del corso da verificare'),
            'excludeid' => new external_value(PARAM_INT, 'ID configurazione da escludere (per modalità edit)', VALUE_DEFAULT, null),
        ]);
    }

    public static function check_course_name_exists_returns() {
        return new external_single_structure([
            'exists' => new external_value(PARAM_BOOL, 'True se il corso esiste già'),
            'count' => new external_value(PARAM_INT, 'Numero di corsi trovati'),
